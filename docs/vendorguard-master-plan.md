...
4. Test M12 approval-bypass scenarios - DONE. Finding review and risk acceptance bypass scenarios were already covered by reviews.test.ts (8 tests: unauthenticated, wrong-role, missing-justification). The questionnaire review/approve gate (added this same session as part of the RBAC audit) had zero test coverage, so added apps/api/src/questionnaireApproval.test.ts with 10 new tests covering: unauthenticated rejection, wrong-role rejection, the separation-of-duties check specifically (the original submitter, holding a REVIEWER role, is blocked from reviewing or approving their own submission), cross-tenant rejection (404, not leaking existence), and the legitimate success path for a different REVIEWER. All 16 typecheck tasks and full test suite pass (API suite now 23 tests total, up from 13). PR #63 merged with all 6 CI checks passing.
5. Test MCP authorization regression - DONE. Audited apps/mcp-server/src/index.ts (12 read-only tools) and context.ts. Confirmed the MCP server never uses assertOwnedByTenant or requirePermission (the two functions changed this session), so there was no regression risk from those changes; every tenant-scoped Prisma query already has tenantId baked directly into its where clause via context.tenantId, and resolveMcpContext re-verifies real TenantMembership from the database rather than trusting what the session cookie claims. Existing context.test.ts (6 tests) already covered the context-resolution layer in isolation, including a test proving a tampered cookie claiming a fake role gets overridden by the real DB membership role. The gap found: no test exercised a full tool call end to end through the real MCP protocol and transport. Closed it with apps/mcp-server/src/authRegression.test.ts (4 tests) using the actual modelcontextprotocol SDK client and streamable HTTP transport against a live test server instance: unauthenticated rejection, legitimate same-tenant success, cross-tenant denial proven through the real wire protocol rather than just code review, and tool allowlist enforcement for an unlisted tool name. Required a small structural change to index.ts (export the server instance and guard the automatic listen call so tests can start and stop it cleanly, with no behavior change in production). All 16 typecheck tasks and full test suite pass, mcp-server suite now 10 tests up from 6. PR #65 merged with all 6 CI checks passing.
6. Fix AI citation relevance - DONE. packages/ai-client/src/assistant.ts previously returned every evidence excerpt present in the AI's context as a citation, regardless of whether the model actually used it to answer - misleading, and actively wrong whenever the answer was flagged insufficientEvidence (it still listed citations for an answer that admitted it had none). Fixed by numbering each evidence excerpt in the system prompt and requiring the model to self-report which excerpt numbers it actually relied on via a trailing CITED_EXCERPTS line, parsed out server-side by a new exported extractCitedExcerpts function and never shown to the user. Citations are always forced empty when insufficientEvidence is true, as a defensive backstop independent of what the model reports. Added assistant.test.ts with 7 new unit tests covering normal parsing, empty citations, the core relevance guarantee (an excerpt only counts if the model actually names it), missing/malformed lines, invalid entries, and matching only the final line. All 16 typecheck tasks and full test suite pass. PR #68 merged with all 6 CI checks passing. This was the last of Phase A's 6 Production Security Blocker items - Phase A is now fully complete.
